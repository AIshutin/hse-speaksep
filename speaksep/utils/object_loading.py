...
import speaksep.augmentations
import speaksep.datasets
from speaksep import batch_sampler as batch_sampler_module
from speaksep.collate_fn.collate import collate_fn
...
def get_dataloaders(configs: ConfigParser):
    ...
    for split, params in configs["data"].items():
        ...
        if split == 'train':
            wave_augs, spec_augs = speaksep.augmentations.from_configs(configs)
            drop_last = True
        else:
        ...
        for ds in params["datasets"]:
            datasets.append(configs.init_obj(
                ds, speaksep.datasets, config_parser=configs,
