from collections.abc import Callable # Python 3.10 stuff
from typing import List

import speaksep.augmentations.spectrogram_augmentations
import speaksep.augmentations.wave_augmentations
from speaksep.augmentations.sequential import SequentialAugmentation
...
def from_configs(configs: ConfigParser):
    ...
    if "augmentations" in configs.config and "wave" in configs.config["augmentations"]:
        for aug_dict in configs.config["augmentations"]["wave"]:
            wave_augs.append(
                configs.init_obj(aug_dict, speaksep.augmentations.wave_augmentations)
            )
    ...
    if "augmentations" in configs.config and "spectrogram" in configs.config["augmentations"]:
        for aug_dict in configs.config["augmentations"]["spectrogram"]:
            spec_augs.append(
                configs.init_obj(aug_dict, speaksep.augmentations.spectrogram_augmentations)
            )
