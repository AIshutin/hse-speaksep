# based on https://github.com/XuMuK1/dla2023/blob/2023/week05/semSpSep.ipynb
import os
import glob
...
def download_librispeech(path: Path):
    path.mkdir(parents=True, exist_ok=True)
    download_file("https://www.openslr.org/resources/12/dev-clean.tar.gz", 
                  path / "train-clean-100.tar.gz", True, replace_existing=True)
    shutil.move(path / "LibriSpeech" / "dev-clean", path / "dev-clean")
    download_file("https://www.openslr.org/resources/12/train-clean-360.tar.gz", 
                  path / "train-clean-360.tar.gz", True, replace_existing=True)
    shutil.move(path / "LibriSpeech" / "train-clean-360", path / "train-clean-360")
    download_file("https://www.openslr.org/resources/12/test-clean.tar.gz", 
                  path / "test-clean.tar.gz", True, replace_existing=True)
    shutil.move(path / "LibriSpeech" / "test-clean", path / "test-clean")


if __name__ == "__main__":
    import argparse

    parser = argparse.ArgumentParser("Mixture dataset generation")
    parser.add_argument("--librispeech", help="path to librispeech dataset", 
                        type=Path,
    ...
    parser.add_argument("--out", type=os.PathLike, help="output path",
                        default=Path("../datasets/libri_ss"))
    parser.add_argument("--n_workers", type=int, default=4)
    parser.add_argument("--update_step", type=int, default=100)
    parser.add_argument("--trim_db", type=int, default=20)
    parser.add_argument("--vad_db", type=int, default=20),
    parser.add_argument("--audio_length", type=int, default=3)
    parser.add_argument("snr_levels", type=int, nargs="+", default=[-5, 5])
    args = parser.parse_args()
    path_train = args.librispeech / "train-clean-360"
    path_val   = args.librispeech / "dev-clean"
    path_test  = args.librispeech / "test-clean"

    if not os.path.exists(args.librispeech):
        download_librispeech(args.librispeech)

    for mixer_args in ("train", False, args.n_train, path_train), \
                      ("val", True, args.n_val, path_val), \
                      ("test", True, args.n_test, path_test):
        speaker_files =[ ]
        for speaker_folder in os.listdir(mixer_args[-1]):
            speaker_files.append(LibriSpeechSpeakerFiles(speaker_folder, 
                                                         mixer_args[-1], 
                                                         audioTemplate="*.flac"))

        mixer = MixtureGenerator(speaker_files, os.path.join(args.out, mixer_args[0]), 
                                 nfiles=mixer_args[2], test=mixer_args[1], randomState=42)
        mixer.generate_mixes(snr_levels=args.snr_levels,
                            num_workers=args.n_workers,
                            update_steps=args.update_step,
                            trim_db=args.trim_db,
                            vad_db=args.vad_db,
                            audioLen=args.audio_length)
